...
# functions for image preproccessing mutate jpg file to numpy arrays for further model prediction
...
def framed_concat(img_1, img_2, img_3, img_4, A, B, C):
    ...
    # B - высота каждого изображения (heigh)
...
def photos_in_frame (heigh:int, width:int, frame_width:int, directory_path, *fnames):

    # Функция приводит 4 фотографии к размерам width, heigh,
    ...
    img_1 = img_1.resize((width, heigh))
    img_2 = img_2.resize((width, heigh))
    img_3 = img_3.resize((width, heigh))
    img_4 = img_4.resize((width, heigh))
    num_1, num_2, num_3, num_4 = [Image.open(os.path.join(num_path, fname)) for
                                  fname in sorted(os.listdir(num_path))[:4]]

    min_dim = min(width, heigh)
    step = 30 # ~ 1 / (отступ квадратика с цифрой от края рамки)

    num_1 = num_1.resize((min_dim // 5, min_dim // 5))
    img_1.paste(num_1, (width - mwh//step - num_1.size[0], heigh - min_dim//step - num_1.size[1]))

    num_2 = num_2.resize((min_dim // 5, min_dim // 5))
    img_2.paste(num_2, (width - mwh//step - num_2.size[0], heigh - min_dim//step - num_2.size[1]))

    num_3 = num_3.resize((min_dim // 5, min_dim // 5))
    img_3.paste(num_3, (width - mwh//step - num_3.size[0], heigh - min_dim//step - num_3.size[1]))

    num_4 = num_4.resize((min_dim // 5, min_dim // 5))
    img_4.paste(num_4, (width - mwh//step - num_4.size[0], heigh - min_dim//step - num_4.size[1]))
    ...
    return Image.fromarray(np.uint8(framed_concat(img_1, img_2, img_3, img_4, width, heigh, frame_width)))
